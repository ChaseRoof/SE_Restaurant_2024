...
document.addEventListener('DOMContentLoaded', async () => {
  const excludedPages = ['/index.html', '/signup.html','/home.html','/post.html','/dishcovery.html'];//had issue where any page would load error needed to exclude pages
  const currentPage = window.location.pathname;

  if (excludedPages.includes(currentPage)) {
    console.log('Skipping username check for:', currentPage);
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error('Unexpected error fetching user profile. Status:', response.status);
  ...
  try {
    ...
    if (response.ok) {
      ...
      reviews.forEach((review) => {
        ...
        `;
        postsContainer.appendChild(reviewDiv);
      });
    } else if (response.status === 404) {
      console.warn('No reviews found for this user.');
      document.getElementById('postsContainer').innerHTML = '<p>No reviews to display.</p>';
    } else {
      console.error('Failed to fetch user reviews. Status:', response.status);
    }
  } catch (error) {
    console.error('Error fetching user reviews:', error);
    document.getElementById('postsContainer').innerHTML = '<p>Could not load reviews. Please try again later.</p>';
  }
}
